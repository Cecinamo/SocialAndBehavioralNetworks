...
public class NewLPA implements Runnable {
    ...
    public static int bestLabel(int[] neighborhood) {
        ...
        int[] nneighborhood = Arrays.copyOfRange(neighborhood, c, neighborhood.length);
        // se non ne è rimasta nessuna
        if (nneighborhood.length == 0) {
            return 0;
        }
        // se è una sola, quella vince
        if (nneighborhood.length == 1) {
            return nneighborhood[0];
        }
        int best = -1; // perché -1 esiste veramente
        int maxCount = 0;
        int counter = 0;
        int last = -1; // perché -1 esiste veramente
        for (int i = 0; i < nneighborhood.length; i++) {
            ...
            if ((maxCount - counter) > (nneighborhood.length - i)) {
                break;
            }

            // se è una nuova etichetta
            if (nneighborhood[i] != last) {
                counter = 1;
                last = nneighborhood[i];
            } else {
                counter++;
            }
